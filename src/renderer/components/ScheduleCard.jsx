    ...
    <div style={{
      ...
      <div style={{ position: 'relative', flexShrink: 0 }}>
        <img
          src={item.thumbnail}
          alt={item.title}
          style={{ width: '160px', height: '90px', objectFit: 'cover', borderRadius: '4px', display: 'block' }}
        />
        {isLive && (
          <span style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>{item.channelTitle}</div>
        <div style={{ fontSize: '12px', color: '#444', marginBottom: '4px' }}>
          {isLive ? `配信中（${formatTime(item.actualStartTime)}〜）` : `${formatTime(item.scheduledStartTime)}〜`}
