...
```bash
npm run lint    # ESLint エラー・warning が 0 件であること
...
このリポジトリは Codex と Claude Code が共同で開発する。汎用ハーネス本体と YouTom 固有 profile は以下を読む。
...
1. ユーザーの依頼を聞いたら、汎用ハーネスと YouTom profile の担当境界で Codex に振るか自分で握るか判断
