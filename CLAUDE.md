...
| トリガー               | 処理                                                             | クォータ               |
| ---------------------- | ---------------------------------------------------------------- | ---------------------- |
| 起動時・`schedule:get` | キャッシュ優先（24時間 TTL）、期限切れなら RSS 取得               | 0〜36 ユニット         |
...
- **TTL: 24時間**。起動時に期限切れキャッシュは破棄して再取得する。
- キャッシュ返却時に `filterStale()` で古い配信を除外する：
  - upcoming: `scheduledStartTime > now` のもののみ
  - live: `actualStartTime` があり、開始から 24時間以内のもののみ
