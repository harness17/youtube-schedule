...
**RssFetcher:**
8. 正常: 200 + 有効 XML で videoId リストが返る
9. 異常系: 404 で `{ success: false, reason: 'http_404' }`
10. 異常系: 3 秒タイムアウトで `{ success: false, reason: 'timeout' }`
11. 異常系: XML パース失敗で `{ success: false, reason: 'parse' }`
12. 境界値: 空フィード（entry 0 件）で `{ success: true, videoIds: [] }`

**SchedulerService:**
13. 正常: RSS 全成功シナリオで videos.list 呼び出し回数が最小化される
14. 正常: RSS 一部失敗 → 失敗 ch だけ playlistItems.list が呼ばれる
15. 権限: OAuth 失効時は Fetcher に到達せずバナー表示用イベント発火
16. 冪等性: 同じデータで 2 回 refresh しても DB に変化なし
17. 競合: refresh 中にもう一度 refresh されても二重実行されない（ロックガード）

**マイグレーション:**
18. 旧 `scheduleCache` が存在する状態で起動 → videos テーブルに転記される
19. 転記後 `scheduleCache` キーが削除される
20. 失敗しても `schema_version` は更新されない（再試行可能）
21. 新規インストール（store 空）でも正常起動
